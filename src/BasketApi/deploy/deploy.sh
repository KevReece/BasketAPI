...
register_definition() {
    ...
    if revision=$(aws ecs register-task-definition --requires-compatibilities FARGATE --cpu 256 --memory 512 --network-mode awsvpc --execution-role-arn $EXECUTION_ROLE_ARN --container-definitions "$task_def" --family $ECS_TASK_FAMILY_NAME | $JQ '.taskDefinition.taskDefinitionArn'); then
